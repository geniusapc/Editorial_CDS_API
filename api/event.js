...
router.post("/", [auth, admin],  async(req, res)=>{ 
    ...
    if (!req.files || !req.files.imagefile) return res.status(400).send("Image is required")    
    ...
    let [data, created] = await Event.findOrCreate({where:{slugTitle}, defaults:{imageName, text, title, createdAt:date}})
...
router.delete("/:id",  [auth, admin], async(req, res)=>{
    ...
    fs.unlink(path.postFolder+`/${event.imageName}`, (e)=>{if(e)res.status(500).send(e)});
...
router.put("/:slugTitle", [auth, admin], async(req, res)=>{
    ...
    if(!!req.files && !!req.files.imagefile){
        fs.unlink(path.postFolder+`/${event.imageName}`, (e)=>{if(e) return res.status(500).send(e) }) 
        let {imagefile, uploadPath, imageName, mimetype } = getImgProps(req.files);
        let {error} = Event.validationImage({mimetype})
        if(error) return res.status(422).send( error.details[0].message);
        await Event.uploadImage(imagefile, uploadPath); 
...
router.put('/reaction/:slugTitle', auth, async(req, res)=>{ 
    let  
    // let {slugTitle} = req.params; 
    // let event =await Event.findOne({where:{slugTitle}});
    // if(!event) return res.status(400).send('Event not found');
...
router.put('/reaction/like/:slugTitle', auth, async(req, res)=>{ 
    let {slugTitle} = req.params; 
    let event =await Event.findOne({where:{slugTitle}});
    if(!event) return res.status(400).send('Event not found');
...
router.put('/reaction/dislike/:slugTitle', auth, async(req, res)=>{ 
    let {slugTitle} = req.params; 
    let event =await Event.findOne({where:{slugTitle}});
    if(!event) return res.status(400).send('Event not found');
    
    if(event.dislikes.includes(req.user.id)) return res.status(400).send("user already disliked this page");
    let dislikes = [...event.dislikes, req.user.id]
    
    await Event.update({dislikes}, {where:{slugTitle}} )
    return  res.status(200).send("success")           
});
...
let getImgProps = (file)=>{
    let {imagefile, imagefile:{mimetype}, imagefile:{name}} = file;
    let date = new Date().toISOString().replace(/:/g, "");
            
    let imageName = `${date}${name}`;
