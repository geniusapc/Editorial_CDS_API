...
router.post("/registration", async(req, res)=>{
    ...
    let {error} = User.validateRegistration({ stateCode, password, confirmPassword })
...
router.post("/login", async(req, res)=>{
        ...
        let token = User.generateAuthToken({ id:user.id, isAdmin:user.isAdmin, isSuperAdmin:user.isSuperAdmin, stateCode:user.stateCode });
        return res.status(200).header("x-auth-token", token).send("login successful")
});
...
router.post('/permission/:rank/:id', [auth, admin, editor],  async(req, res)=>{
    ...
    if(user.role==="EDITOR" && editorCount <= 1 && rank !== "EDITOR"){
        return res.status(400).send("You are not allowed to change your rank unless there is a new editor");
    }
...
router.delete('/:id', [auth, admin, editor], async(req, res)=>{
    ...
    if(user.role==="EDITOR" && editorCount <= 1){
        return res.status(400).send("You are not allowed to delete yourself unless there is a new editor");
    }
