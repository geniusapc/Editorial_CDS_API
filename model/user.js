...
Users.beforeCreate((user, option) => {
  return Users.hashedPassword(user.password).then(hashPw => {
...
Users.verifyPassword = async (password, dbpasswd) =>
  await bcrypt.compare(password, dbpasswd);
...
Users.validateRegistration = schemaCompare => {
  ...
  let stateCodeErr =
    "State code is required and must be of this pattern 'OY/19B/0000' (in uppercase)";
...
Users.validateLogin = schemaCompare => {
  let stateCodeErr =
    "State code is required and must be of this pattern 'OY/19B/0000' (in capitals)";
